...
public record QuestionCreateDto : IEquatable<QuestionCreateDto>
{
    public string? Title { get; init; }
    public int Order { get; init; }
    public List<OptionUpdateDto> options { get; init; }
    public bool Equals(OptionUpdateDto? other)
    {
		...
		bool options_eq = options.OrderBy(x=>x).SequenceEqual<OptionUpdateDto>(options.OrderBy(x=>x));
...
public class UpdateContestCommandHandler : IRequestHandler<UpdateContestCommand, int>
{
	...
	public async Task<int> Handle(UpdateContestCommand request, CancellationToken cancellationToken)
	{
		...
		var QOs_Changed =! oldEntity.Questions.Select(x=>new QuestionCreateDto { Title=x.Title, options=x.Options.Select(y=>new OptionUpdateDto { Title=y.Title, Text=y.Text }).ToList() })
        ...
        if (QOs_Changed )
        {
            ...
            {
                throw new ValidationException(new List<ValidationFailure> { new ValidationFailure() { ErrorCode = "881", ErrorMessage = "This contest has participations and you can't update the questions or options." } });
            }
		...
		_context.Contests.Add(oldEntity);
